...
| G8 | **Same auth as `web:spa`.** Bearer token from `.env` → httpOnly cookie. No new mechanism. | Code review: `backend/app/auth.py` matches the template's pattern within trivial diffs. |
...
- **Auth.** Same bearer-from-env → httpOnly cookie pattern as the `web:spa` template. Cookie name, samesite, secure flags all settings-driven.
...
- **NF-4. Security.** httpOnly cookie. `SameSite=Lax`. `Secure` in production. CSP headers via Caddy. No `dangerouslySetInnerHTML` on streamed content. Markdown rendered through `markdown-it` with link sanitization.
