...
export function ToolPolicyBadge({ policy }: Props) {
  ...
  const { turn_groups, ceiling_groups, reasoning, confidence, fell_back_to_ceiling, cost_usd, latency_ms } =
    policy;
  const isFallback = fell_back_to_ceiling;
  const groupsLabel =
    turn_groups.length === 0 ? "no tools" : turn_groups.join(", ");
  ...
  return (
    <div className="mb-2">
      <button
        ...
        title={
          isFallback
            ? "Planner abdicated — using full ceiling"
